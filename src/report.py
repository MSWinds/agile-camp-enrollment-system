...
class ReportFrame:
    ...
    def run(self):
        window = sg.Window('CSV Viewer', self.layout)
        ...
        while True:
            ...
            if event == 'Load Folder':
                parent_dir = os.path.dirname(os.path.abspath(__file__))  # specify the parent folder of the CSV files
            ...
            if event == 'Load CSV':
                ...
                table_layout = [
                    [ReportFrame.create_table_element(df)],
                    [sg.Button('Close')]
                ]
